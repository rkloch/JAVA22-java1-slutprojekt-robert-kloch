...
class GUI {
	...
	private static void addWeekDay(JPanel groupContainer, LocalDate today, int i) {
		...
		addButtonListener(button, noteContainer, textField, day.equals(today) && today.equals(LocalDate.now()));
		container.add(noteContainer);
		container.add(subContainer, BorderLayout.SOUTH);
		setBackgroundOnPanel(container ,day.equals(today) && today.equals(LocalDate.now()));
		setBackgroundOnPanel(subContainer ,day.equals(today) && today.equals(LocalDate.now()));
		setBackgroundOnPanel(labelContainer ,day.equals(today) && today.equals(LocalDate.now()));
		setBackgroundOnPanel(noteContainer ,day.equals(today) && today.equals(LocalDate.now()));
		groupContainer.add(container);
	}

	private static void setBackgroundOnPanel(JPanel p, boolean isToday) {
		if(isToday) {
	...
	private static void addButtonListener(JButton b, JPanel p, JTextField tf, boolean isToday) {
